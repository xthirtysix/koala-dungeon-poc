...
import ObstaclesPageSkeleton from './obstacles-page-skeleton.vue'
import ObstaclesGridSkeleton from './obstacles-grid-skeleton.vue'
import { obstaclesPageQueryOptions } from '../api/obstacles-page.query'
import { SKELETON_COUNT } from '../consts/obstacles-page.consts'
...
const {
    ...
    hasNextPage,
    error,
} = useInfiniteQuery(obstaclesPageQueryOptions())
...
watch(
    () => [obstacles.value, isFetching.value] as const,
    () => {
        scheduleScrollMarginsUpdate()
    },
    { deep: true },
...
const unlockedContainerOffset = computed(() => {
    ...
    return firstItem.start - unlockedVirtualizer.value.options.scrollMargin
})
...
const trailingRowsCount = computed(() =>
    lockedRowsCount.value > 0 ? lockedRowsCount.value : unlockedRowsCount.value,
)
...
<template>
    <obstacles-page-skeleton v-if="isFetching && !isFetchingNextPage" />
    <template v-else>
        <h1 class="kd-h1 mb-3">Помехи</h1>
        <div v-if="error" class="py-4 text-red-500">{{ error }}</div>
        <div v-else class="relative">
            <h2
                v-if="unlockedRowsCount > 0"
                class="kd-h2 font-amatic text-3xl font-bold first:pt-0"
            >
                Открытые
            </h2>
            <div
                v-if="unlockedRowsCount > 0"
                ref="unlockedListAnchorRef"
                class="relative mb-[3rem]"
            >
                <div
                    :style="{
